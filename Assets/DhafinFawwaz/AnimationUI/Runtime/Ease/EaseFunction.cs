...
// Credit to LitMotion
// https://github.com/AnnulusGames/LitMotion/blob/main/src/LitMotion/Assets/LitMotion/Runtime/EaseUtility.cs
    ...
    public static class EaseFunction
    {
        ...
        public static float Evaluate(float t, Ease ease)
        {
            return ease switch
            {
                ...
                Ease.OutBackQuad => OutBackQuad(t),
                Ease.OutBackCubic => OutBackCubic(t),
                Ease.OutBackQuart => OutBackQuart(t),
                _ => t,
            };
        }
        ...
        [BurstCompile]
        public static float OutBackQuad(float x) => OutBackPow(x, 2);

        [BurstCompile]
        public static float OutBackCubic(float x) => OutBackPow(x, 3);

        [BurstCompile]
        public static float OutBackQuart(float x) => OutBackPow(x, 4);

        [BurstCompile]
        static float OutBackPow(float x, float p)
        {
           return -(x-1)*(x-1)*(x-1)*(x-1) + p*(x-1)*(x-1)*(x-1) + p*(x-1)*(x-1) + 1;
        }
    }
}